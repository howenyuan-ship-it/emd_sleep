...
本專題配合 **2026 年 9 月 24 日黃鍔院士校內專題演講**製作。網站內容於演講舉行前依公開文獻整理而成，未包含演講內容。

## 未來規劃

以下功能**尚未上線**，為開發方向而非現有服務；目前網站不接受任何檔案上傳，亦未設置會員系統。

規劃以會員制開放使用者上傳自有生理訊號進行 EMD 分解分析：

- **訊號類型**：EEG、ECG、ACC、PPG、SpO₂ 等連續生理訊號，上傳時指定類型以套用對應前處理參數
- **檔案格式**：`.txt`、`.csv` 等純文字序列，以及 `.edf`（European Data Format）
- **分析輸出**：各階 IMF 波形、瞬時頻率與振幅、希爾伯特頻譜、各 IMF 能量占比，結果可下載
- **內容更新**：持續追蹤 EMD 在睡眠分期、紡錘波偵測、慢波–紡錘波耦合等主題的新文獻

### 重要聲明

**非醫療用途。** 分析結果僅供學術研究與教學參考，**未經醫療專業人員鑑定評估，不具任何診斷效力**，不得作為診斷、治療、用藥或其他醫療決策依據。健康疑慮請諮詢合格醫師或睡眠專科醫療人員。

**方法限制。** EMD 為資料驅動的經驗性方法，分解結果受取樣頻率、訊號長度、雜訊水準與端點效應影響，不同參數設定可能產生不同 IMF。使用者應自行評估分解結果的合理性。

**資料倫理與隱私。** 請勿上傳可識別特定個人的臨床資料。涉及人體受試者的生理訊號，其蒐集與再利用應符合研究倫理審查（IRB／REC）規範及個人資料保護相關法令；上傳前請確認已取得適當授權並完成去識別化。
...
- **EDF 解析與篩選動畫**：瀏覽器端直接讀取 EDF 二進位檔，逐步動畫呈現 sifting 的每個步驟
- **理論說明**：涵蓋 EMD 篩選算法、HHT 頻譜、各睡眠期特徵
- **學術引用**：15 篇參考文獻，分三組並附重點摘要與 DOI 連結

## EDF 示範

`sample_ecg_acc.edf` 為**程式合成的模擬訊號**，並非任何人的實際生理紀錄，由 `make_sample_edf.py` 產生（腳本一併附上以供檢視與重現）。

| 通道 | 取樣率 | 單位 | 內容 |
|---|---|---|---|
| ECG | 256 Hz | µV | PQRST 複合波，心率約 62 bpm，含 0.25 Hz 呼吸性竇性心律不整（RSA）與基線漂移 |
| ACC_X / ACC_Y | 32 Hz | g | 姿勢擺動與雜訊 |
| ACC_Z | 32 Hz | g | 約 1 g 重力 + **0.25 Hz 呼吸性振盪** + 兩次體動爆發 |

總長 60 秒。網頁端自行實作 EDF 解析（檔頭欄位、int16 → 實體單位換算），不依賴任何函式庫。

刻意在 ACC_Z 注入 0.25 Hz 呼吸成分，是為了驗證 EMD 能否將其分離。實測結果：該成分出現在 IMF4–IMF5（各約 0.25 Hz，換算每分鐘 15 次），與注入值相符；跨兩階 IMF 則呈現 EMD 典型的**模態混疊**現象，正好作為引入 EEMD／CEEMDAN 的實例。殘餘項承載約 1 g 的重力直流分量。

篩選動畫會自動挑選訊號中最平穩的一段並扣除直流偏移——否則體動或重力分量會撐開縱軸，包絡線將擠成一條線而無法判讀。
